...
public class SocketServer {
     ...
     */
    private static CopyOnWriteArraySet<ClientEntity> socketServersList = new CopyOnWriteArraySet<>();
    ...
    public void open(
        ...
        socketServersList.add(new ClientEntity(userName, session));
        //每次连接只要不是管理员就可以进行操作
        if (!userName.equals(SYS_USERNAME)) {
    ...
    public void onMessage(String message) {
        ClientEntity clientEntity = socketServersList
                .stream()
    ...
    public synchronized List<String> getOnlineUsers() {
        //除去管理员
        return socketServersList.stream()
                .filter(clientEntity -> !clientEntity.getUserName().equals(SYS_USERNAME))
